-- PortfolioGuardian FATCA/CRS Step 4 Migration
...
-- 1. Ensure the fatca_crs_owners column exists and is properly configured
DO $$ 
BEGIN
...
BEGIN
    ...
    IF NEW.fatca_crs_owners IS NOT NULL AND array_length(NEW.fatca_crs_owners, 1) > 8 THEN
...
BEGIN
    RETURN (
        SELECT COALESCE(array_length(fatca_crs_owners, 1), 0)
        FROM applications 
        WHERE id = app_id
    );
END;
...
BEGIN
    RETURN (
        SELECT ARRAY(
            SELECT (owner->>'fullLegalName')::TEXT
            FROM applications, 
                 unnest(fatca_crs_owners) AS owner
            WHERE id = app_id
            AND owner->>'fullLegalName' IS NOT NULL
            AND owner->>'fullLegalName' != ''
        )
    );
END;
$$ LANGUAGE plpgsql;
...
SELECT 
    ...
    CASE 
        WHEN a.fatca_crs_owners IS NULL OR array_length(a.fatca_crs_owners, 1) = 0 THEN 'No beneficial owners'
...
BEGIN
    ...
    SET fatca_crs_owners = (
        SELECT ARRAY(
            SELECT owner
            FROM unnest(fatca_crs_owners) AS owner
            WHERE owner->>'fullLegalName' IS NOT NULL 
            AND owner->>'fullLegalName' != ''
        )
    )
...
-- 9. Update RLS policies to include beneficial owners data
-- Note: This assumes you're using the service role for API access
CREATE POLICY "applications_beneficial_owners_view" ON applications
    FOR SELECT USING (
...
-- 11. Add audit logging for beneficial owner changes
...
BEGIN
    ...
    IF (OLD.fatca_crs_owners IS DISTINCT FROM NEW.fatca_crs_owners) THEN
        INSERT INTO audit_logs (
            ...
            NOW(),
            jsonb_build_object(
                'old_count', COALESCE(array_length(OLD.fatca_crs_owners, 1), 0),
                'new_count', COALESCE(array_length(NEW.fatca_crs_owners, 1), 0),
...
-- 13. Create sample query examples (commented out)
/*
-- Example queries for beneficial owners:

-- Get all applications with beneficial owners
SELECT * FROM applications_with_beneficial_owners WHERE beneficial_owner_count > 0;

-- Get applications with specific beneficial owner
SELECT a.*, owner->>'fullLegalName' as owner_name
FROM applications a, unnest(fatca_crs_owners) as owner
WHERE owner->>'fullLegalName' ILIKE '%John%';

-- Get ownership percentage totals
SELECT 
    id,
    reference_number,
    SUM((owner->>'ownershipPercentage')::NUMERIC) as total_ownership
FROM applications a, unnest(fatca_crs_owners) as owner
WHERE owner->>'ownershipPercentage' IS NOT NULL
GROUP BY id, reference_number;

-- Clean up empty beneficial owner entries
SELECT clean_empty_beneficial_owners();
*/
